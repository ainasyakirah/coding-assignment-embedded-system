...
int main(void)
{
  ...
  if (error_code != NEAI_OK) {
 	  	  //this happens if the knowledge does not correspond to the library
 	  	  //or if the library works into a not supported board.
   }
   	 ...
   	 for (uint16_t i = 0; i < DATA_INPUT_USER; i++) {
   			 ...
   			 if (ret != HAL_OK) {
   				 ...
   				 if (ret != HAL_OK) {
   					...
   					input_user_buffer[AXIS_NUMBER * i] = ((buf[1] << 8) | buf[0]);
   					input_user_buffer[(AXIS_NUMBER * i) + 1] = ((buf[3] << 8) | buf[2]);
   					input_user_buffer[(AXIS_NUMBER * i) + 2] = ((buf[5] << 8) | buf[4]);
   				 }
   			 }
   		 }
   		 //print every line of data infinite
   		/* for (uint16_t isample = 0; isample < (NB_AX * BUFFER_SZ - 1); isample++) {
   			 sprintf((char*)buf,"%f ", acc_buffer[isample]);
   			 HAL_UART_Transmit(&huart2, buf, strlen((char*)buf), HAL_MAX_DELAY);
   		 }
   		 sprintf((char*)buf, "%f\r\n", acc_buffer[NB_AX * BUFFER_SZ - 1]);
   		 HAL_UART_Transmit(&huart2, buf, strlen((char*)buf), HAL_MAX_DELAY);*/
